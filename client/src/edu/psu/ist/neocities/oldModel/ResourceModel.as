package edu.psu.ist.neocities.oldModel
{
	...
	public function getResource ( ID : String ) : ResourceVO {
		...
		for (var i : int = 0; i <= data.length - 1; i++){
			if (data.getItemAt(i).resourceID == ID){
				...
				//resource = new ResourceVO ('99', "1", "FAIL", 10);	
			}											
		}
		return resource;
	}
	...
	//used by the AnswerEditor to compare resources to the given Answers, returns a list of unused Resources
	public function getUnusedResources( answerArray : ArrayCollection ) : ArrayCollection {
		var resources : ArrayCollection = new ArrayCollection();
		//if the answer is the same length as the main list, then return the new empty list. 
		if (answerArray.length != data.length){			
			//copy the Array, you can't just do equal assignment operator as it will pass the data by reference
			//we don't want a reference to the data, we just want a copy!	
			for (var k : int = 0; k <= data.length - 1; k++){
				resources.addItem(data.getItemAt(k));
			}	
			
			var total : int = resources.length			
		
			//if the answer is empty, then no need to compare the arrays
			if(answerArray.length != 0) {
				...
				for(var i : int = 0; i < total; i++){ 
					...
					AnswerLoop:
					for (var j : int = 0; j < answerArray.length; j++){
						if (answerArray.getItemAt(j).resourceID == resources.getItemAt(i).resourceID) {
			...
			}//end of empty answer check
		}//end of full answer check	
		return resources;		
	}
	...
	public function isResource ( resourceID : String ) : Boolean {
